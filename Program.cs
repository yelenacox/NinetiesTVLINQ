...
namespace NinetiesTV
{
    class Program
    {
        ...
        **************************************************************************************************/
        ...
        {
            return shows.Where(s => s.EndYear >= 2000)
            .OrderBy(s => s.EndYear).FirstOrDefault();
        }
        ...
        static Show BestDrama(List<Show> shows)
        {
            ...
            .OrderByDescending(s => s.ImdbRating).FirstOrDefault();
        }
        ...
        {
            return shows.Where(s => s.Genres.Contains("Drama"))
            .OrderByDescending(s => s.ImdbRating).Skip(1).ToList();
        }
        ...
        {
            return string.Join(", ", shows.Select(s => s.Name).Take(shows.Count - 1)) + " and "
            + shows.Last().Name;
        }
